...
class Pngx__Cache implements ArrayAccess {
	...
	public static function setup() {
		wp_cache_add_non_persistent_groups( array( 'pngx-non-persistent' ) );
	}
	...
	public function get( $id, $expiration_trigger = '', $default = false, $expiration = 0, $args = array() ) {
		$group = in_array( $id, $this->non_persistent_keys ) ? 'pngx-non-persistent' : 'pngx=posts';
	...
	public function delete( $id, $expiration_trigger = '' ) {
		return wp_cache_delete( $this->get_id( $id, $expiration_trigger ), 'pngx-posts' );
	}
	...
	public function get_last_occurrence( $action ) {
		return (int) get_option( 'pngx_last_' . $action, time() );
	}
	...
	public function set_last_occurrence( $action, $timestamp = 0 ) {
		...
		update_option( 'pngx_last_' . $action, (int) $timestamp );
	}
