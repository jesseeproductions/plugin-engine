...
use Pngx\Utilities\Dates\Date_I18n;
use Pngx\Utilities\Dates\Date_I18n_Immutable;
...
class Utils {
	...
	public static function get_first_day_of_week_in_month( $curdate, $day_of_week ) {
		...
		while ( ! ( $day_of_week > 0 && date( 'N', $nextdate ) == $day_of_week ) && ! ( $day_of_week == - 1 && self::is_weekday( $nextdate ) ) && ! ( $day_of_week == - 2 && self::is_weekend( $nextdate ) ) ) {
			$nextdate = strtotime( date( self::DBDATETIMEFORMAT, $nextdate ) . ' + 1 day' );
		}
	...
	public static function number_to_ordinal( $number ) {
		$output = $number . ( ( ( strlen( $number ) > 1 ) && ( substr( $number, - 2, 1 ) == '1' ) ) ? 'th' : date( 'S', mktime( 0, 0, 0, 0, substr( $number, - 1 ), 0 ) ) );

		return apply_filters( 'pngx_events_number_to_ordinal', $output, $number );
	}
	...
	public static function is_all_day( $all_day_value ) {
		...
		return ( 'true' === strtolower( $all_day_value ) || 'yes' === strtolower( $all_day_value ) || true === $all_day_value || 1 == $all_day_value );
	}
	 ...
	 * @todo       remove in 4.1
	 *
	 * @deprecated since 3.10 - use pngx_event_end_of_day()
	 */
	public static function endOfDay( $date, $isTimestamp = false ) {
		_deprecated_function( __METHOD__, '3.10', 'pngx_event_end_of_day' );

		if ( $isTimestamp ) {
			$date = date( self::DBDATEFORMAT, $date );
		}

		return pngx_event_end_of_day( $date, self::DBDATETIMEFORMAT );
	}
	...
	public static function beginningOfDay( $date, $isTimestamp = false ) {
		_deprecated_function( __METHOD__, '3.10', 'pngx_event_beginning_of_day' );

		if ( $isTimestamp ) {
			$date = date( self::DBDATEFORMAT, $date );
		}

		return pngx_event_beginning_of_day( $date, self::DBDATETIMEFORMAT );
	}
	...
	public static function get_weekday_timestamp( $day_of_week, $week_in_month, $month, $year, $week_direction = 1 ) {
		if ( ! ( is_numeric( $day_of_week ) && is_numeric( $week_in_month ) && is_numeric( $month ) && is_numeric( $year ) && is_numeric( $week_direction ) && in_array( $week_direction, [
				- 1,
				1
			] ) ) ) {
			return false;
	...
	public static function build_date_object( $datetime = 'now', $timezone = null, $with_fallback = true ) {
		...
		try {
			...
			$timezone_object = Pngx__Timezones::build_timezone_object( $timezone );
			...
			return $with_fallback ? new Date_I18n( 'now', $timezone_object ) : false;
		}
	...
	public static function is_valid_date( $date ) {
		...
		$cache_date_check = pngx_get_var( $cache_var_name, [] );
		...
		pngx_set_var( $cache_var_name, $cache_date_check );
	...
	public static function get_week_start_end( $date, $start_of_week = null ) {
		...
		$cache_week_start_end = pngx_get_var( $cache_var_name, [] );
		...
		$cache_key = md5( __METHOD__ . serialize( [ $date_obj->format( static::DBDATEFORMAT ), $week_start_day ] ) );
		$cache     = pngx( 'cache' );
		 ...
		 */
		$week_start->setISODate( (int) $week_start->format( 'o' ), (int) $week_start->format( 'W' ) + $week_offset, $week_start_day );
		...
		pngx_set_var( $cache_var_name, $cache_week_start_end );
	 ...
	 *
	 * It's the immutable version of the `Pngx__Date_Utils::build_date_object` method.
	 *
	 * @since 4.10.2
	 *
	 * @param string|DateTime|int      $datetime      A `strtotime` parse-able string, a DateTime object or
	 ...
	 * Builds a date object from a given datetime and timezone.
	 *
	 * An alias of the `Pngx__Date_Utils::build_date_object` function.
	 *
	 * @since 4.10.2
	 *
	 * @param string|DateTime|int      $datetime      A `strtotime` parse-able string, a DateTime object or
