...
class Pngx__Log {
	...
	/**
	 * @var Pngx__Log__Admin
	 */
	protected $admin;

	/**
	 * @var Pngx__Log__Logger
	 */
	...
	public function __construct() {
		if ( is_admin() ) {
			$this->admin = new Pngx__Log__Admin();
		}
	 ...
	 * implementation (making it safe to call Pngx__Log__Logger methods on the
	 * return value of this method at all times).
	 *
	 * @since 4.6.2 altered the return signature to only return instances of Pngx__Log__Logger
	 *
	 * @return Pngx__Log__Logger
	 */
	public function get_current_logger() {
		if ( ! $this->current_logger ) {
			...
			if ( empty( $engine ) || ! isset( $available[ $engine ] ) ) {
				return $this->current_logger = new Pngx__Log__Null_Logger();
			} else {
	...
	public function get_engine( $class_name ) {
		if ( ! isset( $this->loggers[ $class_name ] ) ) {
			...
			if ( $object instanceof Pngx__Log__Logger ) {
	...
	public function set_level( $level ) {
		...
		pngx_update_option( 'logging_level', $level );
	...
	public function get_logging_levels() {
		if ( empty( $this->levels ) ) {
			 ...
			 * is critical, see Pngx__Log::get_logging_levels() docs.
			 *
			 * General form:
			 *
			 *     [
