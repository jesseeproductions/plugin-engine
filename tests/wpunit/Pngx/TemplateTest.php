...
use Pngx\Tests\Dummy_Plugin_Origin;
...
class TemplateTest extends \Codeception\TestCase\WPTestCase {
	...
	public function should_include_entry_points_on_template_html() {
		...
		add_action( 'pngx_template_entry_point:dummy/dummy-template:after_container_open', function () {
			echo '%%after_container_open%%';
		} );
		add_action( 'pngx_template_entry_point:dummy/dummy-template:before_container_close', function () {
	...
	public function should_include_custom_entry_points_on_template_html() {
		...
		add_action( 'pngx_template_entry_point::custom_entry_point', function () {
		...
		$last_tag_html             = '</div>';
		$html                      = $template->template( 'dummy-template', [], false );
		$html                      = \Pngx\Utilities\Strings::replace_last( $last_tag_html, $last_tag_html . $customer_entry_point_html, $html );
	...
	public function should_not_include_with_invalid_html() {
		...
		add_action( 'pngx_template_entry_point:dummy/dummy-invalid-template-01:after_container_open', function () {
			echo '%%after_container_open%%';
		} );
		add_action( 'pngx_template_entry_point:dummy/dummy-invalid-template-01:before_container_close', function () {
			echo '%%before_container_close%%';
		} );
	...
	public function should_not_include_with_invalid_html_02() {
		...
		add_action( 'pngx_template_entry_point:dummy/dummy-invalid-template-02:after_container_open', function () {
			echo '%%after_container_open%%';
		} );
		add_action( 'pngx_template_entry_point:dummy/dummy-invalid-template-02:before_container_close', function () {
			echo '%%before_container_close%%';
		} );
	...
	public function should_not_include_with_invalid_html_03() {
		...
		add_action( 'pngx_template_entry_point:dummy/dummy-invalid-template-03:after_container_open', function () {
			echo '%%after_container_open%%';
		} );
		add_action( 'pngx_template_entry_point:dummy/dummy-invalid-template-03:before_container_close', function () {
			echo '%%before_container_close%%';
		} );
	...
	public function should_not_include_with_invalid_html_04() {
		...
		add_action( 'pngx_template_entry_point:dummy/dummy-invalid-template-04:after_container_open', function () {
			echo '%%after_container_open%%';
		} );
		add_action( 'pngx_template_entry_point:dummy/dummy-invalid-template-04:before_container_close', function () {
			echo '%%before_container_close%%';
		} );
	...
	public function should_include_with_valid_html() {
		...
		add_action( 'pngx_template_entry_point:dummy/dummy-valid-template-01:after_container_open', function () {
			echo '%%after_container_open%%';
		} );
		add_action( 'pngx_template_entry_point:dummy/dummy-valid-template-01:before_container_close', function () {
			echo '%%before_container_close%%';
		} );
		$html = $template->template( 'dummy-valid-template-01', [], false );

		$this->assertContains( '<a href="https://pngx.com" class="test" target="_blank" title="Test Link" data-link="automated-tests">%%after_container_open%%', $html );
	...
	public function should_include_with_valid_html_02() {
		...
		add_action( 'pngx_template_entry_point:dummy/dummy-valid-template-02:after_container_open', function () {
			echo '%%after_container_open%%';
		} );
		add_action( 'pngx_template_entry_point:dummy/dummy-valid-template-02:before_container_close', function () {
			echo '%%before_container_close%%';
		} );
	...
	public function should_include_with_valid_html_03() {
		...
		add_action( 'pngx_template_entry_point:dummy/dummy-valid-template-03:after_container_open', function () {
			echo '%%after_container_open%%';
		} );
		add_action( 'pngx_template_entry_point:dummy/dummy-valid-template-03:before_container_close', function () {
			echo '%%before_container_close%%';
		} );
		$html = $template->template( 'dummy-valid-template-03', [], false );

		$replaced_html = str_replace( array( "\n", "\r" ), '', $html );
		$this->assertContains( '<div class="pngx-view pngx-view--base pngx-view--dummy">%%after_container_open%%', $replaced_html );
		$this->assertStringEndsWith( '%%before_container_close%%</div>', $html );
	}
	...
	public function should_not_include_with_entry_points_disabled() {
		...
		add_action( 'pngx_template_entry_point_is_enabled', '__return_false' );
	...
	public function should_add_common_template_path_when_looking_for_files() {
		...
		$assert = function ( array $folders ) {
			...
			$this->assertEquals( \Pngx__Main::instance()->plugin_path . 'src/views', $folders['common']['path'] );
		};
		add_filter( 'pngx_template_path_list', $assert, PHP_INT_MAX );
	...
	public function should_allow_setting_aliases_w_common_lookup() {
		...
		$assert = function ( array $folders ) {
			...
			$this->assertEquals( __DIR__ . '/test-plugin/src/views/v4_2', $folders['plugin']['path'] );
			$this->assertEquals( __DIR__ . '/test-plugin/src/views/v4', $folders['plugin_v4']['path'] );
			$this->assertEquals( \Pngx__Main::instance()->plugin_path . 'src/views/v4_2', $folders['common']['path'] );
			$this->assertEquals( \Pngx__Main::instance()->plugin_path . 'src/views/v4', $folders['common_v4']['path'] );
			$this->assertEquals(
	...
	public function should_allow_using_aliases_to_rewrite_path_fragments() {
		...
		$assert = function ( array $folders ) {
			...
			$this->assertEquals( \Pngx__Main::instance()->plugin_path . 'src/templates/v3_1',
				$folders['common']['path'] );
			$this->assertEquals( \Pngx__Main::instance()->plugin_path . 'src/views/v3',
	...
	public function should_allow_filtering_template_html_and_echo_it(): void {
		...
		add_filter( 'pngx_template_pre_html:template/say-hi',
